...
def gen_data(n_examples, n_numbers, max_value):
    ...
    for _ in range(n_examples):
        x = [random.random() * max_value for _ in range(n_numbers)]  # n_numbers = 2면 [x1, x2] 형태의 데이터 생성
        y = x[0]
        for j in range(1, n_numbers):  # n_numbers값에 따라 for문이 돌면서 뒤에 입력한 값들 계산
            if symbol == '+':   # 여기 부분에서 어떤 연산을 하냐에 따라 달라짐
                y = y + x[j]
            elif symbol == '-':
                y = y - x[j]
            elif symbol == '*':
    ...
    # numpy 배열로 변환
    input_value, output_value = np.array(input_value), np.array(output_value).reshape(n_examples, 1)
